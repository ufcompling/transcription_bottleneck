...
root_dir="/blue/liu.ying/word_making/data/"

sdir=/blue/liu.ying/asr_resource/kaldi/tools/srilm/bin/i686-m64
...
for subfolder in "$root_dir"/*/; do
    ...
    if [ -d "$split_dir" ]; then
        ...
        for category in min max rand; do
            ...
            $sdir/ngram-count -order 3 -write-vocab $split_dir/vocab-full.txt -wbdiscount -text $output_file -lm $split_dir/$(basename ${output_file} .txt).arpa
