...
#	print('Vocab size:', len(vocab_dict))
	...
	repo_name = 'model/' + lang + '/' + size + '/al/' + seed + '/' + select_interval + '/select' + select + '/' + pretrained_model
	print(repo_name) 
...
#	test_data = [tok for tok in test_data_temp if tok is not None]
	...
	## Make sure to keep these numbers consistent across languages
	## Which might be less ideal for some languages
	## But for our cross-linguistic experiments, we want to have more controll
	epochs = 30
	batch_size = 16
...
def main():
	...
	if select not in ['0', 'all']:
		previous_datadir = data_path + '/' + size + '/' + method + '/' + seed + '/' + select_interval + '/select' + str(int(select) - int(select_interval)) + '/'

	if select == '0':
		original_data = pd.read_csv(data_path + 'train_' + seed + '.csv')
		...
		i = 0
		while initial_train_dur <= int(size) * 60 and i < len(combined_data):
			tok = combined_data[i]
			dur = float(tok[-2])
			temp_dur += float(dur)
			if temp_dur <= int(size) * 60:
				initial_train_dur = temp_dur
...
#				select_dur += float(tok[-2])
		...
		initial_train_output.to_csv(sub_datadir + pretrained_model + '_train.' + size + '.input', index = False)
		...
		select_output.to_csv(sub_datadir + pretrained_model + '_select.' + size + '.input', index = False)
...
#		os.system('cat ' + previous_datadir + 'train.' + size + '.input ' + previous_datadir + 'increment.input >' + sub_datadir + 'train.' + size + '.input')
	...
	train_data = ''
	if select not in ['all']:
		print(sub_datadir + pretrained_model + '_train.' + size + '.input')
		train_data = get_data_reg(sub_datadir, pretrained_model + '_train.' + size + '.input', pretrained_model, select)
	else:
		train_data = get_data_reg(data_path, 'train_' + seed + '.csv', pretrained_model, select)
